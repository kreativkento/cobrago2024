...
<body>
    ...
    <center>
        <h1>COLLEGE DEANS</h1>
        <button class="add-account-button"><a href="./dean_account_add_form.html">Add Account</button></a>
        ...
        $sql = "SELECT * FROM dean_account";

        $txtsearch = '';
        if (isset($_POST['btnsearch'])) {
            $txtsearch = $_POST['txtsearch'];
            $sql .= "WHERE dean_id LIKE '%$txtsearch%' OR 
                    dean_firstname LIKE '%$txtsearch%' OR 
                    dean_lastname LIKE '%$txtsearch%' OR 
                    dean_email LIKE '%$txtsearch%'";
        } else {
            $sql .= "";
        }

        $sql .= " ORDER BY dean_id, dean_pass, dean_firstname, dean_lastname, dean_email";
            ...
            <tbody>
                ...
                if (mysqli_num_rows($result) == 0) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        echo "<tr>";
                        echo "<td>$dean_id</td>";
                        echo "<td>$dean_pass</td>";
                        echo "<td>$dean_firstname</td>";
                        echo "<td>$dean_lastname</td>";
                        echo "<td>$dean_email</td>";
                        echo "<td><a class='edit-button' href='dean_account_edit_form.php?dean_id=$dean_id'>EDIT</a> | 
                             <a class='delete-button' href='dean_account_delete_confirmation.php?dean_id=$dean_id'>DELETE</a></td>";
                        echo "</tr>";
                    }
                }
                ?>
