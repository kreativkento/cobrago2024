...
<body>
    ...
    <center>
        <h1>STUDENT LIFE USERS</h1>
        <button class="add-account-button"><a href="./sl_account_add_form.html">Add Account</button></a>
        ...
        $sql = "SELECT * FROM sl_account";

        $txtsearch = '';
        if (isset($_POST['btnsearch'])) {
            $txtsearch = $_POST['txtsearch'];
            $sql .= "WHERE sl_id LIKE '%$txtsearch%' OR 
                    sl_firstname LIKE '%$txtsearch%' OR 
                    sl_lastname LIKE '%$txtsearch%' OR 
                    sl_email LIKE '%$txtsearch%'";
        } else {
            $sql .= "";
        }

        $sql .= " ORDER BY sl_id, sl_pass, sl_firstname, sl_lastname, sl_email";
            ...
            <tbody>
                ...
                if (mysqli_num_rows($result) == 0) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        echo "<td>$sl_id</td>";
                        echo "<td>$sl_pass</td>";
                        echo "<td>$sl_firstname</td>";
                        echo "<td>$sl_lastname</td>";
                        echo "<td>$sl_email</td>";
                        echo "<td><a class='edit-button' href='sl_account_edit_form.php?sl_id=$sl_id'>EDIT</a> | 
                             <a class='delete-button' href='sl_account_delete_confirmation.php?sl_id=$sl_id'>DELETE</a></td>";
                        echo "</tr>";
                    }
                }
                ?>
