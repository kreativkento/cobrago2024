...
<body>
    <div class="container">
        <h1><a href="./guidance_account.php">EDIT GUIDANCE USER ACCOUNT</a></h1>
        ...
        if(isset($_GET['go_id'])) {
            $id = $_GET['go_id'];
            $sql = "SELECT * FROM go_account WHERE go_id='$id'";
            $result = mysqli_query($con, $sql) or die("Error in SQL statement, please check coder under SELECT");

            while ($row = mysqli_fetch_array($result)) {
                $id = $row['go_id'];
                $pass = $row['go_pass'];
                $last = $row['go_firstname'];
                $first = $row['go_lastname'];
                $email = $row['go_email'];
            }
        }
        ?>

        <form method="post" action="./guidance_account_edit_process.php?id=<?php echo $id; ?>">
            <table>
