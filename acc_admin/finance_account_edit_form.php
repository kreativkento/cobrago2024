...
<body>
    <div class="container">
        <h1><a href="./finance_account.php">EDIT FINANCE USER ACCOUNT</a></h1>

        <?php
        include '../connection.php';

        $id = '';
        if(isset($_GET['fin_id'])) {
            $id = $_GET['fin_id'];
            $sql = "SELECT * FROM fin_account WHERE fin_id='$id'";
            $result = mysqli_query($con, $sql) or die("Error in SQL statement, please check coder under SELECT");

            while ($row = mysqli_fetch_array($result)) {
                $id = $row['fin_id'];
                $pass = $row['fin_pass'];
                $last = $row['fin_firstname'];
                $first = $row['fin_lastname'];
                $email = $row['fin_email'];
            }
        }
        ?>

        <form method="post" action="./finance_account_edit_process.php?id=<?php echo $id; ?>">
            <table>
