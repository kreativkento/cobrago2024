...
<body>
    <div class="container">
        <h1><a href="./dean_account.php">EDIT DEAN USER ACCOUNT</a></h1>
        ...
        if(isset($_GET['dean_id'])) {
            ...
            while ($row = mysqli_fetch_array($result)) {
                $id = $row['dean_id'];
                $pass = $row['dean_pass'];
                $last = $row['dean_firstname'];
                $first = $row['dean_lastname'];
                $email = $row['dean_email'];
            }
        }
        ?>

        <form method="post" action="./dean_account_edit_process.php?id=<?php echo $id; ?>">
            <table>
